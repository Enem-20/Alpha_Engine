...
public:
	static glm::vec3 CellToScreen(const glm::ivec2& ivec2, const glm::ivec2& cellSize)
	{
		...
		float scaleX = static_cast<float>(RenderEngine::Renderer::ViewportSize.x / cellSize.x);
		...
		float scaleY = static_cast<float>(RenderEngine::Renderer::ViewportSize.y / cellSize.y);
